...
#  0x0000000000400f20 <+36>:    callq  0x40143a <explode_bomb> #爆炸
   ...
   0x0000000000400f2c <+48>:    jne    0x400f17 <phase_2+27> #不相等就跳转到 +27
...
0x4025cf:    "%d %d"
   ...
   0x0000000000401033 <+39>:    jbe    0x40103a <phase_4+46> #低于或相等跳转到+46
   0x0000000000401035 <+41>:    callq  0x40143a <explode_bomb> #不低于14就爆炸
   ...
   0x000000000040104f <+67>:    jne    0x401058 <phase_4+76> #不等于0就跳转
   0x0000000000401051 <+69>:    cmpl   $0x0,0xc(%rsp)  #判断第二个数是否为0
   0x0000000000401056 <+74>:    je     0x40105d <phase_4+81> #相等就跳转
...
Dump of assembler code for function func4:
   ...
   0x0000000000400fe4 <+22>:    jle    0x400ff2 <func4+36> # 小于或等于就跳转到 +36
   ...
   0x0000000000400ff9 <+43>:    jge    0x401007 <func4+57> # 大于或等于0 则跳转到 +57
...
Dump of assembler code for function phase_5:
   ...
   0x0000000000401082 <+32>:    je     0x4010d2 <phase_5+112> # 相等跳转到　+112
   ...
   0x00000000004010ac <+74>:    jne    0x40108b <phase_5+41> # 不相等则 跳转到 +41
...
# 开启循环
   ...
   0x0000000000401121 <+45>:    jbe    0x401128 <phase_6+52> # 低于或等于5就跳转
   0x0000000000401123 <+47>:    callq  0x40143a <explode_bomb> 
   0x0000000000401128 <+52>:    add    $0x1,%r12d  #　%r12d+1
   0x000000000040112c <+56>:    cmp    $0x6,%r12d  # 比较是否等于6，6次循环
...
   0x0000000000401132 <+62>:    mov    %r12d,%ebx # %ebx = %r12d
   ...
   0x0000000000401140 <+76>:    callq  0x40143a <explode_bomb> # 若6个数字有相等则爆炸
   ...
   0x000000000040114d <+89>:    add    $0x4,%r13  # %r13 + 4 # 加4
   0x0000000000401151 <+93>:    jmp    0x401114 <phase_6+32> # 跳转到+32

# 可以看出6个数字都是正整数，存在栈中 ，都不大于6且不相等
...
# 又是一个循环
   ...
   0x0000000000401166 <+114>:    add    $0x4,%rax # %rax + 4  取栈中下一个数字
   0x000000000040116a <+118>:    cmp    %rsi,%rax #  是否到栈底
   0x000000000040116d <+121>:    jne    0x401160 <phase_6+108> # 不相等就跳转+108 继续循环

# 这个循环是用这6个数做减数，被减数是7，差值存入原位置

# 又一个循环
   0x000000000040116f <+123>:    mov    $0x0,%esi　# %esi = 0
   0x0000000000401174 <+128>:    jmp    0x401197 <phase_6+163>

   0x0000000000401176 <+130>:    mov    0x8(%rdx),%rdx # 指向下一个节点地址
   ...
   0x0000000000401191 <+157>:    cmp    $0x18,%rsi # 比较 %rsi
   0x0000000000401195 <+161>:    je     0x4011ab <phase_6+183>  #若%rsi的值等于24则跳转

   0x0000000000401197 <+163>:    mov    (%rsp,%rsi,1),%ecx # ecx = %rsp + %rsi*1
   0x000000000040119a <+166>:    cmp    $0x1,%ecx  # 比较%ecx是否为1
   0x000000000040119d <+169>:    jle    0x401183 <phase_6+143> # 小于或等于则跳转到 +143
   0x000000000040119f <+171>:    mov    $0x1,%eax  # %eax = 1
   0x00000000004011a4 <+176>:    mov    $0x6032d0,%edx # %edx = $0x6032d0
   0x00000000004011a9 <+181>:    jmp    0x401176 <phase_6+130>

# 判断读取的数字是否为1，若是就将0x6032d0放在栈中，若不是则用该数字作为节点序号将对应链表节点地址存在栈中
链表节点序号对应读取到的数值
ex:
   链表第一个节点为node1，对应0x6032d0,
   若当前的数字为3，则将链表第三个节点node3对应的地址0x6032e0存储到栈中，依次类推
...
0x603320 <node6>:       0x000001bb      0x00000006      0x00000000       0x00000000
   ...
   0x00000000004011ab <+183>:    mov    0x20(%rsp),%rbx    # %rbx = %rsp+0x20  链表第一个节点
   0x00000000004011b0 <+188>:    lea    0x28(%rsp),%rax    # %rax = %rsp+0x28  栈中的链表第三个节点位置
   0x00000000004011b5 <+193>:    lea    0x50(%rsp),%rsi    # 链表尾部
   0x00000000004011ba <+198>:    mov    %rbx,%rcx   # %rcx = %rbx

   0x00000000004011bd <+201>:    mov    (%rax),%rdx  # %rdx = (%rax) 取链表节点位置
   0x00000000004011c0 <+204>:    mov    %rdx,0x8(%rcx) # (%rcx+0x8) = %rdx
   0x00000000004011c4 <+208>:    add    $0x8,%rax   # %rax+0x8  
   0x00000000004011c8 <+212>:    cmp    %rsi,%rax   # 检查链表是否到尾部
   ...
   0x00000000004011df <+235>:    mov    0x8(%rbx),%rax  # %rax = %rbx+0x8 取下一个节点在栈中位置
   0x00000000004011e3 <+239>:    mov    (%rax),%eax   # %eax=%rax
   0x00000000004011e5 <+241>:    cmp    %eax,(%rbx) # (%rbx) - eax  比较下一个节点是否大于当前节点
   0x00000000004011e7 <+243>:    jge    0x4011ee <phase_6+250> # 大于或等于时跳到 +250
   0x00000000004011e9 <+245>:    callq  0x40143a <explode_bomb> 
   0x00000000004011ee <+250>:    mov    0x8(%rbx),%rbx  # %rbx = %rbx+0x8 next
   0x00000000004011f2 <+254>:    sub    $0x1,%ebp # %ebp - 1
   0x00000000004011f5 <+257>:    jne    0x4011df <phase_6+235> #非零就跳转
...
大致流程是输入6个数字，用7作差的值重新对链表进行调整（数值对应链表的节点序号），调整后的链表节点值是单调递减的。

```shell
查看 0x6032d0 及对应存储的节点值
x /24dw 0x6032d0
