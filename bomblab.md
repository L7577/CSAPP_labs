...
如果是第一个数是0，就跳转到 `0x400f7c`，通过计算间接跳转地址`8 * 0 + *0x402470 = *0x402470`可知。其他情况亦可照此计算。注意是带`*`运算符的，在`0x402470`处存着`0x400f83`。
...
Dump of assembler code for function func4:
   ...
   0x0000000000400ff9 <+43>:	jge    0x401007 <func4+57> # 不是0 则跳转到 +57
...
可以看到 func4函数，自己调用了自己，就是个递归函数，边界条件是什么呢，就是 `cmp    %edi,%ecx`就是第一个数,存储在`%edi`，等于`%ecx`的值，才可以退出递归，并且第一个数不能是0

在 `0x400fe2`处打断点，输入参数为 `1 0`
...
Dump of assembler code for function phase_6:
   ...
   0x000000000040114b <+87>:	jle    0x401135 <phase_6+65> # 相等就跳转到 +65
   0x000000000040114d <+89>:	add    $0x4,%r13  # %r13 + 4
   0x0000000000401151 <+93>:	jmp    0x401114 <phase_6+32>

# 可以看出6个数字都是正数，存在栈中  从%rsp 到 %rsp+0x14
...
# 这个循环是，用这6个数做减数，被减数是7
   ...
   0x0000000000401176 <+130>:	mov    0x8(%rdx),%rdx # 取到第二个数
   ...
   0x000000000040119d <+169>:	jle    0x401183 <phase_6+143> # 若是则跳转到 +143
