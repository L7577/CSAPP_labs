...
使用位运算，返回补码最小值，，需注意`tmin==~tmax`，`int`是32位，补码最小值就是符号位（最高位）为1，其余全是0，即是`10000000 00000000 00000000 00000000`，可将`0x1` 末尾的1左移31位到最高位即可。
...
如果是补码最大值则返回1，但需注意不能使用if判断，所以只能返回一个表达式的值；那就只剩下一种方式了，假定是补码最大值，将其转换成全0，再用逻辑非`!`运算符取反，返回其布尔值1，若不是补码最大值，则返回的布尔值就是0了。
...
正常情况下，`补码最大值+1` 就是 `10000000 00000000 00000000 00000000` 再加上补码最大值得到全1，也就是 `11111111 11111111 11111111 11111111`,再进行逻辑非运算，可以转换到全0；但需要注意 对于 -1， 补码为 `11111111 11111111 11111111 11111111`，对其加1，得到全0`00000000 00000000 00000000 00000000`，再与-1相加也是全1，有所冲突了，如何排除呢，需要上面的基础上再做一次判断，观察-1+1，就已经是全0了，，对其逻辑非运算会得到1，但对`补码最大值+1`的逻辑非操作是0，全0+0还是全0，其他值则会得到1。再返回其逻辑非运算后的值，是补码最大值就为1，不是则为0，这下也就可以排除-1这种情况了。
...
```
用`0xaa`与上x，判断第八位，再用0xaa与上x>>8  ，依次类推，每次判断八位，
